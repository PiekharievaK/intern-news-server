...
export const adRequestController = async (
  ...
  for (const bid of bids) {
    ...
    const lineItems = await fastify.prisma.lineItem.findMany({
      where: {
        size: {
          equals: [width, height],
        },
    ...
    const mathRandom = (length: number) => {
      return Math.floor(Math.random() * length);
    };

    const randomIndex = mathRandom(lineItems.length);
    const lineItem = lineItems[randomIndex];
